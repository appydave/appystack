...
Operational knowledge for a Ralph Wiggum Task Agent campaign. Each agent receives this file + a specific work unit. The template is at `/Users/davidcruwys/dev/ad/apps/appystack/template/`.
...
Launch sequentially. Each agent reads this AGENTS.md, does the work, runs validation, commits, reports.

---

### Phase 1: Foundation Fixes

#### WU-1: Dependency cleanup
- **What**: Remove `formidable` from server/package.json (it's installed but never imported). Audit all three workspaces for other phantom deps by checking imports against package.json dependencies.
- **Why**: Unused dependencies bloat the template and confuse users who expect everything installed to be used.
- **Done when**: `npm install` succeeds. `npm run build` passes. No package in any workspace's dependencies/devDependencies is unimported in source files.
